"use client";

import { useEffect, useRef, useState } from "react";
import Image from "next/image";
import Link from "next/link";

function useReveal(threshold = 0.1) {
  const elRef = useRef<HTMLDivElement>(null);
  const [visible, setVisible] = useState(false);
  useEffect(() => {
    const io = new IntersectionObserver(
      ([e]) => {
        if (e.isIntersecting) setVisible(true);
      },
      { threshold },
    );
    if (elRef.current) io.observe(elRef.current);
    return () => io.disconnect();
  }, [threshold]);
  return { elRef, visible };
}

const STATS = [
  { value: "100+", unit: "hours", label: "Experiential Learning" },
  { value: "8", unit: "cohorts", label: "Since 2021" },
  { value: "118+", unit: "graduates", label: "And Counting" },
  { value: "60%", unit: "female", label: "Participant Makeup" },
];

const PILLARS = [
  {
    number: "01",
    title: "Work Experience",
    body: "12 weeks of practical placements with leading partner institutions — real work, real challenges, real growth.",
  },
  {
    number: "02",
    title: "Self Awareness",
    body: "8 weeks of structured training to develop identity, emotional intelligence, and personal leadership.",
  },
  {
    number: "03",
    title: "Corporate Awareness",
    body: "Exposure to professional environments, workplace culture, and the skills it takes to thrive in industry.",
  },
  {
    number: "04",
    title: "Community",
    body: "Become part of a growing alumni network of purpose-driven young professionals across Ghana and beyond.",
  },
];

export default function AequitasExperiencePage() {
  const { elRef: aboutRef, visible: aboutVisible } = useReveal(0.08);
  const { elRef: statsRef, visible: statsVisible } = useReveal(0.1);
  const { elRef: pillarsRef, visible: pillarsVisible } = useReveal(0.06);
  const { elRef: eligRef, visible: eligVisible } = useReveal(0.1);
  const { elRef: ctaRef, visible: ctaVisible } = useReveal(0.1);
  const [heroVisible, setHeroVisible] = useState(false);

  useEffect(() => {
    const t = setTimeout(() => setHeroVisible(true), 80);
    return () => clearTimeout(t);
  }, []);

  return (
    <main style={{ background: "var(--color-ink)" }}>
      {/* ── Hero ─────────────────────────────────────────────────── */}
      <section
        className="relative overflow-hidden px-6 md:px-16 lg:px-24"
        style={{
          paddingTop: "clamp(100px, 14vw, 160px)",
          paddingBottom: "clamp(60px, 9vw, 100px)",
        }}
      >
        {/* Ghost watermark */}
        <div
          aria-hidden
          className="absolute right-0 top-0 font-extrabold leading-none select-none pointer-events-none"
          style={{
            fontSize: "clamp(80px, 15vw, 220px)",
            color: "rgba(55,197,243,0.03)",
            fontFamily: "var(--font-montserrat)",
            letterSpacing: "-0.04em",
            lineHeight: 1,
          }}
        >
          EXPERIENCE
        </div>

        <div
          className={`relative transition-all duration-700 ${heroVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"}`}
        >
          {/* Breadcrumb */}
          <div className="flex items-center gap-2 mb-5">
            <Link
              href="/"
              className="text-[11px] tracking-[0.15em] uppercase transition-colors duration-200"
              style={{ color: "rgba(255,255,255,0.3)", fontFamily: "'Courier New', monospace" }}
            >
              Home
            </Link>
            <span style={{ color: "rgba(255,255,255,0.15)", fontSize: 11 }}>/</span>
            <span
              className="text-[11px] tracking-[0.15em] uppercase"
              style={{ color: "var(--color-blue)", fontFamily: "'Courier New', monospace" }}
            >
              Aequitas Experience
            </span>
          </div>

          <p className="text-[11px] font-semibold tracking-[0.25em] uppercase text-blue mb-3.5">
            Our Programmes
          </p>
          <h1
            className="font-extrabold text-white"
            style={{
              fontSize: "clamp(2.6rem, 6.5vw, 5.5rem)",
              lineHeight: 1.0,
              fontFamily: "var(--font-montserrat)",
            }}
          >
            The Aequitas<br />
            <span className="text-blue">Experience</span>
          </h1>
          <p
            className="mt-5 text-[15px] leading-[1.85] max-w-lg"
            style={{ color: "rgba(255,255,255,0.45)" }}
          >
            A 12-week immersive programme equipping young Ghanaians with the
            resilience, skills, and community they need to thrive.
          </p>

          <div className="w-12 mt-7" style={{ height: 3, background: "var(--color-blue)" }} />
        </div>
      </section>

      {/* ── About + Image ────────────────────────────────────────── */}
      <section
        style={{ background: "#fff" }}
      >
        <div
          ref={aboutRef}
          className={`grid lg:grid-cols-2 transition-all duration-700 ${aboutVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
        >
          {/* Image column */}
          <div className="relative overflow-hidden" style={{ minHeight: 560 }}>
            <Image
              src="/aequitas-exp-page.webp"
              alt="Aequitas Experience participants on a staircase celebrating"
              fill
              sizes="(max-width: 1024px) 100vw, 50vw"
              className="object-cover object-center"
              priority
            />
            {/* Blue caption bar */}
            <div
              className="absolute bottom-0 left-0 right-0 px-8 py-6"
              style={{ background: "var(--color-blue)" }}
            >
              <p
                className="text-white font-bold leading-snug"
                style={{
                  fontSize: "clamp(1rem, 2vw, 1.2rem)",
                  fontFamily: "var(--font-montserrat)",
                }}
              >
                Creating safe spaces for the youth to find and pursue their passion.
              </p>
            </div>
          </div>

          {/* Text column */}
          <div
            className="px-8 sm:px-12 lg:px-16 xl:px-20 flex flex-col justify-center"
            style={{
              paddingTop: "clamp(48px, 7vw, 80px)",
              paddingBottom: "clamp(48px, 7vw, 80px)",
            }}
          >
            <h2
              className="font-extrabold leading-tight mb-6"
              style={{
                fontSize: "clamp(1.4rem, 2.5vw, 2rem)",
                color: "#0a0e1a",
                fontFamily: "var(--font-montserrat)",
              }}
            >
              A holistic programme that contributes to lifelong success of our
              youth through a resilience-focused approach.
            </h2>

            <div className="flex flex-col gap-4 text-[15px] leading-[1.85]" style={{ color: "#4b5563" }}>
              <p>
                This flagship initiative of the foundation is a 12-week
                internship and training opportunity for the youth — to give them
                a chance to discover who they are, their strengths and passions,
                and exposure to the world of work.
              </p>
              <p>
                The Aequitas Experience is structured to provide beneficiaries
                with over 100 hours of practical work experience with partner
                institutions and 8 weeks of training to build self, others, and
                corporate awareness. With 8 cohorts since its institution in
                2021, the programme has recorded an average participant age of
                26, with 118 graduates and 12–24 participants per cohort.
              </p>
              <p>
                The Aequitas Experience targets GenZs (Aged 18 to 24) as well as
                young graduates seeking full-time employment. We seek out engaged
                and empowered individuals interested in personal and professional
                development.
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* ── Stats ────────────────────────────────────────────────── */}
      <section
        className="px-6 md:px-16 lg:px-24 relative overflow-hidden"
        style={{
          background: "var(--color-ink)",
          paddingTop: "clamp(56px, 8vw, 96px)",
          paddingBottom: "clamp(56px, 8vw, 96px)",
        }}
      >
        {/* Subtle grid texture */}
        <div
          aria-hidden
          className="absolute inset-0 pointer-events-none select-none"
          style={{
            backgroundImage:
              "linear-gradient(rgba(55,197,243,0.03) 1px, transparent 1px), linear-gradient(90deg, rgba(55,197,243,0.03) 1px, transparent 1px)",
            backgroundSize: "64px 64px",
          }}
        />

        <div ref={statsRef} className="relative">
          <p className="text-[11px] font-semibold tracking-[0.25em] uppercase text-blue mb-10">
            Programme at a Glance
          </p>

          <div className="grid grid-cols-2 lg:grid-cols-4 gap-px" style={{ background: "rgba(255,255,255,0.05)" }}>
            {STATS.map((s, i) => (
              <div
                key={s.label}
                className={`px-8 py-10 transition-all duration-500 ${statsVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
                style={{
                  background: "var(--color-ink)",
                  transitionDelay: `${i * 80}ms`,
                }}
              >
                <div
                  className="font-extrabold leading-none mb-1"
                  style={{
                    fontSize: "clamp(2.5rem, 5vw, 4rem)",
                    color: "#fff",
                    fontFamily: "var(--font-montserrat)",
                  }}
                >
                  {s.value}
                </div>
                <div
                  className="text-[11px] font-semibold tracking-[0.2em] uppercase mb-2"
                  style={{ color: "var(--color-blue)" }}
                >
                  {s.unit}
                </div>
                <div
                  className="text-[13px] leading-relaxed"
                  style={{ color: "rgba(255,255,255,0.35)" }}
                >
                  {s.label}
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* ── Programme Pillars ────────────────────────────────────── */}
      <section
        style={{ background: "#0d1017" }}
        className="px-6 md:px-16 lg:px-24"
      >
        <div
          ref={pillarsRef}
          className={`transition-all duration-700 ${pillarsVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
          style={{
            paddingTop: "clamp(56px, 8vw, 96px)",
            paddingBottom: "clamp(56px, 8vw, 96px)",
          }}
        >
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-12">
            <div>
              <p className="text-[11px] font-semibold tracking-[0.25em] uppercase text-blue mb-3">
                How It Works
              </p>
              <h2
                className="font-extrabold text-white"
                style={{
                  fontSize: "clamp(1.8rem, 3.5vw, 3rem)",
                  lineHeight: 1.1,
                  fontFamily: "var(--font-montserrat)",
                }}
              >
                Built on Four<br />Core Pillars
              </h2>
            </div>
            <div className="w-12 mb-1.5" style={{ height: 3, background: "var(--color-blue)" }} />
          </div>

          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-px" style={{ background: "rgba(255,255,255,0.05)" }}>
            {PILLARS.map((p, i) => (
              <div
                key={p.number}
                className={`px-7 py-8 group transition-all duration-500 ${pillarsVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
                style={{
                  background: "#0d1017",
                  transitionDelay: `${i * 80}ms`,
                }}
              >
                <div
                  className="text-[11px] font-semibold tracking-[0.2em] mb-5"
                  style={{
                    color: "var(--color-blue)",
                    fontFamily: "'Courier New', monospace",
                  }}
                >
                  {p.number}
                </div>
                <h3
                  className="font-bold text-white mb-3 leading-tight"
                  style={{
                    fontSize: "clamp(1rem, 1.4vw, 1.15rem)",
                    fontFamily: "var(--font-montserrat)",
                  }}
                >
                  {p.title}
                </h3>
                <p className="text-[13px] leading-[1.8]" style={{ color: "rgba(255,255,255,0.4)" }}>
                  {p.body}
                </p>
                {/* Blue accent line on hover */}
                <div
                  className="mt-6 transition-all duration-300 group-hover:w-full"
                  style={{ width: "2rem", height: 2, background: "var(--color-blue)" }}
                />
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* ── Eligibility ──────────────────────────────────────────── */}
      <section
        className="px-6 md:px-16 lg:px-24"
        style={{
          background: "#fff",
          paddingTop: "clamp(56px, 8vw, 96px)",
          paddingBottom: "clamp(56px, 8vw, 96px)",
        }}
      >
        <div
          ref={eligRef}
          className={`grid lg:grid-cols-2 gap-12 lg:gap-24 transition-all duration-700 ${eligVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
        >
          {/* Left */}
          <div>
            <p
              className="text-[11px] font-semibold tracking-[0.25em] uppercase mb-3"
              style={{ color: "var(--color-blue)" }}
            >
              Who Can Apply
            </p>
            <h2
              className="font-extrabold leading-tight mb-6"
              style={{
                fontSize: "clamp(1.8rem, 3vw, 2.8rem)",
                color: "#0a0e1a",
                fontFamily: "var(--font-montserrat)",
              }}
            >
              Made for the<br />Next Generation
            </h2>
            <div
              className="text-[15px] leading-[1.85] flex flex-col gap-4"
              style={{ color: "#4b5563" }}
            >
              <p>
                The Aequitas Experience is designed for GenZs aged 18 to 24, as
                well as young graduates who are ready to step into the world of
                work but want more than just a job — they want purpose.
              </p>
              <p>
                We seek engaged, open-minded individuals interested in genuine
                personal and professional growth, who are ready to be challenged,
                stretched, and shaped into well-rounded leaders.
              </p>
            </div>
          </div>

          {/* Right — checklist */}
          <div className="flex flex-col justify-center gap-5">
            {[
              "Aged 18–24 (GenZ) or a recent graduate",
              "Ghanaian or based in Ghana",
              "Seeking full-time employment or career clarity",
              "Open to personal development & mentorship",
              "Passionate about contributing to community",
              "No prior work experience required",
            ].map((item, i) => (
              <div key={i} className="flex items-start gap-4">
                <div
                  className="shrink-0 mt-0.5 flex items-center justify-center"
                  style={{
                    width: 22,
                    height: 22,
                    border: "1.5px solid var(--color-blue)",
                    color: "var(--color-blue)",
                    fontSize: 11,
                    fontWeight: 700,
                  }}
                >
                  ✓
                </div>
                <p className="text-[15px] leading-snug" style={{ color: "#374151" }}>
                  {item}
                </p>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* ── CTA ──────────────────────────────────────────────────── */}
      <section
        className="relative overflow-hidden px-6 md:px-16 lg:px-24"
        style={{
          background: "var(--color-ink)",
          paddingTop: "clamp(72px, 10vw, 112px)",
          paddingBottom: "clamp(72px, 10vw, 112px)",
        }}
      >
        {/* Ghost watermark */}
        <div
          aria-hidden
          className="absolute right-0 bottom-0 font-extrabold leading-none select-none pointer-events-none"
          style={{
            fontSize: "clamp(100px, 18vw, 260px)",
            color: "rgba(55,197,243,0.03)",
            fontFamily: "var(--font-montserrat)",
            letterSpacing: "-0.04em",
            lineHeight: 0.85,
          }}
        >
          APPLY
        </div>

        <div
          ref={ctaRef}
          className={`relative flex flex-col lg:flex-row items-start lg:items-end justify-between gap-10 transition-all duration-700 ${ctaVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
        >
          <div className="max-w-xl">
            <p className="text-[11px] font-semibold tracking-[0.25em] uppercase text-blue mb-3.5">
              Ready to Begin?
            </p>
            <h2
              className="font-extrabold text-white leading-[1.05]"
              style={{
                fontSize: "clamp(2rem, 4vw, 3.2rem)",
                fontFamily: "var(--font-montserrat)",
              }}
            >
              Take the First Step<br />
              <span className="text-blue">Towards Your Future</span>
            </h2>
            <p
              className="mt-4 text-[15px] leading-[1.8]"
              style={{ color: "rgba(255,255,255,0.45)" }}
            >
              Applications open periodically throughout the year. Fill in the
              form and our team will be in touch with the next steps.
            </p>
          </div>

          <div className="flex flex-col sm:flex-row gap-4 shrink-0">
            <Link
              href="/apply"
              className="inline-block px-10 py-4 font-bold text-[12px] tracking-[0.15em] uppercase text-ink transition-colors duration-200"
              style={{ background: "var(--color-blue)" }}
            >
              Apply Now
            </Link>
            <Link
              href="/contact"
              className="inline-block px-10 py-4 font-bold text-[12px] tracking-[0.15em] uppercase text-white transition-all duration-200"
              style={{ border: "1px solid rgba(255,255,255,0.15)" }}
            >
              Ask a Question
            </Link>
          </div>
        </div>
      </section>
    </main>
  );
}
