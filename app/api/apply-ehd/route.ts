...
export async function POST(request: NextRequest) {
  ...
  if (
    ...
    console.error("[apply-ehd] Missing Google credentials in environment");
    return NextResponse.json(
  ...
  try {
    ...
    if (cvEntry) {
      ...
      const publicId = `ehd-${fullName}-${Date.now()}`;
    ...
    // ── Append row to EmpowerHerDreams sheet tab ────────────────────────────
    ...
    const row = [
      ...
      "Female", // F — Gender (programme is female-only)
      ...
      sanitize(formData.get("nationalService")), // M — National Service Status
      sanitize(formData.get("employed")), // N — Employment Status
      "Empower Her Dreams", // O — Programme
      sanitize(formData.get("about")), // P — About
      sanitize(formData.get("goals")), // Q — Career Goals
      sanitize(formData.get("skillsNeeded")), // R — Skills / Support Needed
      sanitize(formData.get("linkedin")), // S — LinkedIn (optional)
      sanitize(formData.get("howHeard")), // T — How Did You Hear About Us
      cvLink, // U — CV Link
    ];

    await sheets.spreadsheets.values.append({
      spreadsheetId: process.env.GOOGLE_SHEET_ID,
      range: "EmpowerHerDreams!A:U",
    ...
    console.error("[apply-ehd] Submission error:", err);
    return NextResponse.json(
